...
from src.data.utils import get_visible_mask, get_occlusion_mask, transform, encode_binary_labels
from src.data.nuscenes.utils import *
...
def process_sample_data(nuscenes, map_data, sample_data, lidar, config):
    ...
    output_path = os.path.join(os.path.expandvars(config.nuscenes_label_root),
...
if __name__ == '__main__':
    ...
    nuscenes = NuScenes(config.nuscenes_version, config.nuscenes_dataroot)

    # Preload NuScenes map data
    map_data = { location : load_map_data(config.nuscenes_dataroot, location) for location in LOCATIONS }
    
    # Create a directory for the generated labels
    output_root = os.path.expandvars(config.nuscenes_label_root)
