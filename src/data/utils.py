...
def decode_binary_labels(labels, nclass):  # labels: 1 x 196 x 200, nclass=15
    # bits = tensor([1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384]), length=15
    bits = torch.pow(2, torch.arange(nclass))
    return (labels & bits.view(-1, 1, 1)) > 0  # 15 x 196 x 200
