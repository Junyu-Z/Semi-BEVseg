...
def FPN50():
    fpn = FPN([3,4,6,3])
    fpn.load_pretrained('https://download.pytorch.org/models/resnet50-19c8e357.pth')
    '''
    pretrained = torch.load('/home/ma-user/modelarts/user-job-dir/BEVmix/BEVmix/src/models/resnet50-19c8e357.pth')
    state_dict = fpn.state_dict()
    for key, weights in pretrained.items():
        if key in state_dict:
            state_dict[key].copy_(weights)
...
if __name__ == '__main__':
    backbone = FPN50()
    total_num = sum(p.numel() for p in backbone.parameters())
    print('total_num =', total_num)
    x = torch.rand(1, 3, 600, 800)
    fs = backbone(x)  # 1x256x57x100, 1x256x29x50, 1x256x15x25, 1x256x8x13, 1x256x4x7
    for f in fs:
        print(f.shape)
